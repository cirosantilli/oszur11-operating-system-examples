...
/* Identify UART chip - algorithm taken from:
...
static int uart_config ( device_t *dev, uart_t *params )
{
	...
	ASSERT_ERRNO_AND_EXIT ( params->data_bits >= 5 || params->data_bits<=8,
				E_INVALID_ARGUMENT );
	ASSERT_ERRNO_AND_EXIT ( params->parity >= 0 || params->parity <= 7,
				E_INVALID_ARGUMENT );
	ASSERT_ERRNO_AND_EXIT ( params->stop_bit >= STOPBIT_1 ||
				params->stop_bit <= STOPBIT_15,
				E_INVALID_ARGUMENT );
	ASSERT_ERRNO_AND_EXIT ( params->mode == UART_BYTE ||
