...
void k_threads_init ()
{
	...
	/* setup programs */
	prog.pi->stdin = u_stdin;
	prog.pi->stdout = u_stdout;
	...
	/* first "user" thread */
	k_create_thread (prog.pi->init, NULL, prog.pi->exit, prio, NULL, 0, 1);
...
int sys__create_thread ( void *p )
{
	...
	kthr = k_create_thread ( func, param, prog.pi->exit, prio, NULL, 0, 1 );
