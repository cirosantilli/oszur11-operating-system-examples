...
kprocess_t kernel_proc; /* for idle thread only */
kprocess_t user_proc;
...
void k_threads_init ()
{
	...
	user_proc.pi->stdin = u_stdin;
	user_proc.pi->stdout = u_stdout;
	...
	user_proc.thr_count = 0;

	prio = prog.pi->prio;
	if ( !prio )
		prio = THR_DEFAULT_PRIO;

	k_create_thread ( user_proc.pi->init, NULL, NULL, prio,
				NULL, 0, 1, &user_proc );
