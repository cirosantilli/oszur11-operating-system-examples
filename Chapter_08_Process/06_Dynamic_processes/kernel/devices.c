...
int sys__device_send ( void *p )
{
	...
	data = U2K_GET_ADR ( *( (void **) p ), k_get_active_process() );
	p += sizeof (void *);
...
int sys__device_open ( void *p )
{
	...
	dev_name = U2K_GET_ADR ( *( (char **) p ), k_get_active_process() ); 
	p += sizeof (char *);

	dev = U2K_GET_ADR ( *( (void **) p ), k_get_active_process() );
